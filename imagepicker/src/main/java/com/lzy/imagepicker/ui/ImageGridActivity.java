...
public class ImageGridActivity extends ImageBaseActivity
    ...
    private boolean mIsSkip = false;//是否直接跳转到照相界面
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mIsSkip = getIntent().getBooleanExtra("phonetype", false);
        ...
        mIsAvatar = getIntent().getBooleanExtra("isAvatar", false);
        if (mIsAvatar) {
            int circle = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 300,
                    this.getResources().getDisplayMetrics());
            imagePicker.setFocusWidth(circle);
            imagePicker.setFocusHeight(circle);
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.btn_ok) {
            Intent intent = new Intent(ImageGridActivity.this, PicViewerActivity.class);
            intent.putExtra(ImagePicker.EXTRA_RESULT_ITEMS, imagePicker.getSelectedImages());
            // setResult(ImagePicker.RESULT_CODE_ITEMS, intent); //多选不允许裁剪裁剪，返回数据
            startActivityForResult(intent, ImagePicker.RESULT_CODE_COMPILE);
    ...
    public void onImageItemClick(View view, ImageItem imageItem, int position) {
        ...
        if (imagePicker.isMultiMode()) {
            ...
            imagePicker.addSelectedImageItem(position,
                    imagePicker.getCurrentImageFolderItems().get(position), true);
            if (mIsAvatar) {
            ...
            if (imagePicker.getCropIndex() == 1 || imagePicker.getCropIndex() == 3 || imagePicker.getCropIndex() == 6 || imagePicker.getCropIndex() == 8 || imagePicker.getCropIndex() == 9) {
                ...
                Intent photoFrameIntent = new Intent(ImageGridActivity.this, ImageTailorActivity.class);
                photoFrameIntent.putExtra("camera_path", imagePicker.getSelectedImages().get(0).path);
...
/*            if (imagePicker.isCrop()) {
    ...
    public void onImageSelected(int position, ImageItem item, boolean isAdd) {
        ...
        mBtnPre.setText(
                getResources().getString(R.string.preview_count, imagePicker.getSelectImageCount()));
...
//                startActivityForResult(intent, ImagePicker.REQUEST_CODE_CROP); //单选需要裁剪，进入裁剪界面
                ...
                } else if (imagePicker.getCropIndex() == 1 || imagePicker.getCropIndex() == 3 || imagePicker.getCropIndex() == 6 || imagePicker.getCropIndex() == 8 || imagePicker.getCropIndex() == 9) {
                    // 将图片路径photoPath传到所要调试的模块
                    Intent photoFrameIntent = new Intent(ImageGridActivity.this, ImageTailorActivity.class);
                    photoFrameIntent.putExtra("camera_path", imagePicker.getSelectedImages().get(0).path);
                    startActivityForResult(photoFrameIntent, IMAGE_PICKER2);
                } else {
                    //2017-12-4 Updated by Jenny  拍照完成后，跳转到定制的裁剪页面
